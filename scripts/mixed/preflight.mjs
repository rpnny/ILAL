...
for(const [address,hash] of [[c.poolManager,c.poolManagerCodeHash],[c.verifierAdapter,c.verifierAdapterCodeHash],[c.create2Factory,c.factoryCodeHash]])if(hash&&keccak256(await client.getCode({address}))!==hash)throw new Error('External runtime hash mismatch');
