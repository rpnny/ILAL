/** Generate exact ordered transactions only. This script has NO signing or broadcasting capability. */
...
export function validateDeploymentConfig(c){
 ...
 for(const key of ['deployer','create2Factory','admin','poolManager','token0','token1','verifierAdapter','feed0','feed1','sequencerFeed'])getAddress(c[key]);
 ...
 if(c.classification==='production'){
  ...
  for(const key of ['sourceCommit','sourceDigest','verifierAdapterCodeHash','poolManagerCodeHash','factoryCodeHash','ceremonyTranscriptSHA256','zkeySHA256','vkeySHA256','independentAuditSHA256','issuerAcceptanceSHA256'])if(!new RegExp(key==='sourceCommit'?'^[0-9a-f]{40}$':'^(0x)?[0-9a-f]{64}$').test(c[key]??''))throw new Error(`Production evidence missing: ${key}`);
 }
 return c;
}
export function prepareDeployment(c){
 ...
 const a=artifact('MixedHook'),code=encodeDeployData({abi:a.abi,bytecode:a.bytecode.object,args:[{manager:c.poolManager,oracle,eligibility:grantManager,executionRouter,liquidityRouter}]});let salt,hook;
 for(let i=0n;;i++){salt=toHex(i,{size:32});hook=getCreate2Address({from:c.create2Factory,salt,bytecodeHash:keccak256(code)});if((BigInt(hook)&0x3fffn)===0xaa8n)break;}
 call('Deploy mined Hook using reviewed deploy(bytes32,bytes) factory',c.create2Factory,'MixedLocalFactory','deploy',[salt,code]);
 ...
 return {format:'ilal-mixed-deployment-plan-v1',status:'UNBROADCAST',configuration:c,predicted:{referenceGuard,oracle,policyRegistry,grantManager,executionRouter,liquidityRouter,hook,poolId},transactions,requiredPreflight:['Recheck chain and starting nonce','Verify manager/factory/verifier runtime hashes','Verify ordered token decimals and standard ERC20 behavior','Verify feeds correspond to the named ordered assets, heartbeat, sequencer and thresholds','Verify source revision/digest and ceremony/issuer/audit evidence','Simulate every transaction in order and inspect Hook flags 0x0aa8','Verify canonical bindings, roles, live policy and oracle after execution; do not migrate old signatures']};
}
