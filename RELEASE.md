...
The canonical machine-readable template is `releases/v0.3.3-rc.1.json`. `sourceCommit` remains `null` until clean source freeze. Because a commit cannot contain its own SHA, tracked `releaseCommit` remains `null`; the tag workflow resolves it to `GITHUB_SHA` in the published release asset. Neither value may be guessed or derived from a dirty tree.

## Version compatibility

| Artifact | Meaning |
|---|---|
| Local `cli/` source `0.3.3-rc.1` | New signer abstraction, Safe proposal flow, deprecated preset removal, bounded execution |
